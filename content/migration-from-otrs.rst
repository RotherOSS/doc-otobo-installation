...
.. code-block:: bash
   ...
   cd /opt/otobo
   sudo scripts/backup.pl --backup-type migratefromotrs # it's OK that the command knows only about the otobo database, only last line is relevant
...
.. code-block:: bash
   ...
   cd <dump_dir>
   sudo mysql -u root -p<root_secret> otobo < otrs_pre.sql
   sudo mysql -u root -p<root_secret> otobo < otrs_schema_for_otobo.sql
   sudo mysql -u root -p<root_secret> otobo < otrs_data.sql
   sudo mysql -u root -p<root_secret> otobo < otrs_post.sql
...
.. code-block:: bash
   ...
   cd /opt/otobo-docker
   sudo docker-compose exec -T db mysql -u root -p<root_secret> otobo < /opt/otobo/<dump_dir>/otrs_pre.sql
   sudo docker-compose exec -T db mysql -u root -p<root_secret> otobo < /opt/otobo/<dump_dir>/otrs_schema_for_otobo.sql
   sudo docker-compose exec -T db mysql -u root -p<root_secret> otobo < /opt/otobo/<dump_dir>/otrs_data.sql
   sudo docker-compose exec -T db mysql -u root -p<root_secret> otobo < /opt/otobo/<dump_dir>/otrs_post.sql

For a quick check whether the import worked, you can run the following commands.

.. code-block:: bash

   sudo -u otobo mysql -u root -p<root_secret> -e 'SHOW DATABASES'
   sudo -u otobo mysql -u root -p<root_secret> otobo -e 'SHOW TABLES'
   sudo -u otobo mysql -u root -p<root_secret> otobo -e 'SHOW CREATE TABLE ticket'

or when running under Docker

.. code-block:: bash

   sudo docker-compose exec -T db mysql -u root -p<root_secret> -e 'SHOW DATABASES'
   sudo docker-compose exec -T db mysql -u root -p<root_secret> otobo -e 'SHOW TABLES'
   sudo docker-compose exec -T db mysql -u root -p<root_secret> otobo -e 'SHOW CREATE TABLE ticket'
