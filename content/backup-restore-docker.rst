...
Please read the Backup and Restore description :doc:`backup-restore` for basic information about the backup and restore scripts.

Considerations for running OTOBO under Docker
----------------------------------------------

The scripts ``backup.pl`` and ``restore.pl`` can also be used with OTOBO running under Docker.
...
Because in that case all data would be lost when the container is stopped.
