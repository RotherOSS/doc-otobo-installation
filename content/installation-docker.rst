...
.. code-block:: bash
   ...
   sudo docker create --name tmp-nginx-container rotheross/otobo-nginx-webproxy:latest-10_1
...
.. code-block:: bash
   ...
   sudo docker load --input mariadb:10.5.docker
...
.. code-block:: bash
   ...
   sudo docker-compose stop # if otobo is running
...
.. code-block:: bash
   ...
   sudo docker exec -it ${COMPOSE_PROJECT_NAME:=otobo}_web_1 bash
   cd /opt/otobo
...
.. code-block:: bash
   ...
   sudo docker run -it --user root --entrypoint /bin/bash --name otobo_orig rotheross/otobo:rel-10_1_10
   apt update
