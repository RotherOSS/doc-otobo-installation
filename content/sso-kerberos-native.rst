...
.. note::
    ...
    The SSO kerbeos setup is quite complex. You may stay with LDAP and avoid all the headache.
...
* Otobo trust the username passed in the ``REMOTE_USER`` environment variable 
  or the ``Remote-User`` HTTP header and skips the login screen
...
Register an SPN (Service Principal Name) for the otobo service. Open terminal and run:
...
.. note::
    ...
    Keytab is a cryptographic file containing a representation of the service and its long-term key
    as it exists in the directory service. 
    In our Active Directory domain, keytab file will be used by the Webserver (that runs on Linux)
    to authenticate User against AD via Kerberos protocol.
...
.. note::
    ...
    instructions were proved on Ubuntu 24.04. Other linux flavours can have different names for Kerberos libraries.
...
This is a correct answer. Goto webserver section.
...
Double check your ``kinit`` arguments. If agruments are correct, but you still got this error, it may be the keytab has incorrect SPN inside.
...
.. note::
    ...
    I suppose you already have apache running, if not, read `Otobo basic installation manual <https://doc.otobo.de/manual/installation/10.1/en/content/installation.html#step-5-configure-the-apache-web-server>`_
...
Create apache config: ``nano /etc/apache2/sites-available/otobo-apache-ssl.conf``
